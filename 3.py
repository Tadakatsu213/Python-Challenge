...
try:
    with open(budget_data_path, encoding='UTF-8') as csvfile:
        ...
        for row in csvreader:
            ...
            # Extract the month and profit/loss from the row
            month = row[0]
            profit_loss = row[1]
            
            # Skip rows where profit_loss is an empty string
            if profit_loss == '' or profit_loss == 'Profit/Losses':
                continue
        
            profit_loss = int(profit_loss)
            
            # Add the month and profit/loss to their respective lists
            months.append(month)
            profits_losses.append(profit_loss)
            
            # Increase the month count and total profit/loss
            total_months += 1
